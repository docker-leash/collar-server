...
def create_app(debug=False):
    ...
    app.config.update(
        ...
        LEASH_CA_CERT=os.getenv("LEASH_CA_CERT", app.config["LEASH_CA_CERT"]),
        LEASH_CONNECT_TIMEOUT=os.getenv("LEASH_CONNECT_TIMEOUT", app.config["LEASH_CONNECT_TIMEOUT"]),
        DOCKER_URL=os.getenv("DOCKER_URL", app.config["DOCKER_URL"]),
        DOCKER_CA_CERT=os.getenv("DOCKER_CA_CERT", app.config["DOCKER_CA_CERT"]),
        DOCKER_CERT_FILE=os.getenv("DOCKER_CERT_FILE", app.config["DOCKER_CERT_FILE"]),
        DOCKER_KEY_FILE=os.getenv("DOCKER_KEY_FILE", app.config["DOCKER_KEY_FILE"]),
        DOCKER_CONNECT_TIMEOUT=os.getenv("DOCKER_CONNECT_TIMEOUT", app.config["DOCKER_CONNECT_TIMEOUT"]),
    )
    ...
    def get_collar_username():
        ...
        app.logger.info("Collar username: %s", subject)
        return subject
    ...
    def authz_request():
        ...
        if app.config["ALLOW_PING"] and _ping_matcher.match(payload["RequestUri"]):
        ...
        if payload["RequestMethod"] == "GET" and app.config["ALLOW_READONLY"]:
        ...
        app.logger.debug("CONTACTING LEASH SERVER: %s/AuthZPlugin.AuthZReq" % (app.config["LEASH_URL"]))
